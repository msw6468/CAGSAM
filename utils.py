...
def overlay_mask_on_image(image, mask, color=(0, 255, 0), alpha=0.2, 
                        points=None,
                        original_size = None,
                        image_size = None):
    ...
    mask_bool = mask > 0
    overlay = image.copy()
    overlay[mask_bool.squeeze()] = cv2.addWeighted(image, 1 - alpha, color_mask, alpha, 0)[mask_bool.squeeze()]
    ...
    if points is not None:
        ...
        for point, label in zip(ori_points, point_labels):
            x, y = map(int, point)
            color = (0, 0, 255)
            cv2.drawMarker(overlay, (x, y), color, markerType=cv2.MARKER_CROSS , markerSize=10, thickness=2)  

    return overlay

def save_overlapped_mask(image, mask, save_path, 
                        gt= None, 
                        main_gt = None,
                        image_name = None,
                        image_size = None, 
                        original_size = None, 
                        points=None, alpha=0.2,
                        ):

    if image_name is not None:
        # fine_gt = cv2.imread(glob(f'data/FineARCADE/*/*/fine_masks/{image_name}')[0], 0)
        # main_gt = cv2.imread(glob(f'data/FineARCADE/*/*/main_masks/{image_name}')[0], 0)
        fine_gt = gt - main_gt
        
    overlapped_mask = overlay_mask_on_image(image, mask, color=(0, 255, 0), alpha=alpha, 
                                            original_size = original_size, image_size = image_size, points=points)
    overlapped_gt   = overlay_mask_on_image(image, main_gt, color=(255, 0, 0), alpha=alpha, 
                                            original_size = original_size, image_size = image_size, points=points)
    overlapped_gt   = overlay_mask_on_image(overlapped_gt, fine_gt, color=(0, 255, 255), alpha=alpha, 
                                            original_size = original_size, image_size = image_size, points=points)
    concat_img = np.concatenate([image, overlapped_gt, overlapped_mask], axis=1)
